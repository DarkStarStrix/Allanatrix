...
export const SITE = {
  ...
  description:
    "Machine Learning Engineer at Aethron Labs. Building NexaMol and Nexa_Mat — foundation models for mass spectrometry and materials science — alongside Nexa_Compute (ML infra), Science Gym (RL for scientific decisions), and PyC (AI compiler toolchain for HPC).",
...
export const CURRENT_PROJECTS: Project[] = [
  ...
  },
  {
    id: "nexa-mat",
    title: "Nexa_Mat",
    subtitle: "Materials Science Foundation Model",
    status: "active",
    stage: "Training — Active",
    company: "Aethron Labs",
    description:
      "A foundation model for materials science trained on crystal structure data. Learns equivariant representations of atomic configurations using graph neural networks, targeting energy prediction, force estimation, and electrolyte screening for battery research.",
    highlights: [
      "Equivariant GNN encoder: EGNN, local/angular/tensor potential families",
      "Materials dataset: 87k crystal structures with physics labels",
      "SSB screening: electrolyte-relevant material classification (3-class MLP)",
      "Integrated into Nexa_Compute training infrastructure on Blackwell GPUs",
    ],
    stack: ["Python", "PyTorch", "PyMatGen", "HuggingFace", "CUDA", "DuckDB"],
    metrics: [
      { label: "Dataset", value: "87k rows" },
      { label: "HF Downloads", value: "64" },
      { label: "Model Families", value: "6" },
    ],
    links: [
    ...
    progress: [
      ...
      { id: "model", label: "Encoder Architecture", status: "complete", note: "EGNN + potential families" },
      { id: "ssb", label: "SSB Screening Classifier", status: "complete", note: "3-class MLP, epoch-45 checkpoint" },
      { id: "train", label: "Foundation Model Training", status: "active", note: "encoder_pretrain phase" },
      { id: "eval", label: "Benchmark Evaluation", status: "next", note: "Energy/force prediction metrics" },
    ],
  },
  {
    id: "science-gym",
...
export const TERMINAL_COMMANDS: Record<string, string> = {
  ...
  nexamat: [
    "",
    "  Nexa_Mat — Materials Science Foundation Model",
    "  ─────────────────────────────────────────────",
    "  HuggingFace: huggingface.co/AethronPhantom",
    "  Dataset:     Allanatrix/Materials — 87k rows · 64 downloads",
    "",
    "  Architecture: Equivariant GNN encoder",
    "    Families: EGNN · local_potential · angular_potential",
    "              tensor_potential · equivariant_potential",
    "",
    "  SSB Screening: 3-class MLP classifier",
    "    Classes: hard_negative · general_inorganic · electrolyte_relevant",
    "    Checkpoint: epoch-45 · strict state-dict loading",
    "",
    "  Training phase: encoder_pretrain [ACTIVE]",
    "  Target: energy prediction · force estimation · battery materials",
    "",
  ].join("\n"),
  ...
  status: [
    ...
    "",
    "  [●] Nexa_Mat            TRAINING",
    "      Materials foundation model · 87k crystal structures",
    "      encoder_pretrain phase — Blackwell GPUs",
    "",
